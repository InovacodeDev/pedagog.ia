...
- **Server State:** `TanStack Query` (React Query) v5.
  - _Rule:_ Use for all async server data. Do not use `useEffect` for data fetching.
- **Global Client State:** `Zustand`.
  - _Rule:_ Use minimal atomic stores. Avoid massive monolithic stores.
- **URL State:** `nuqs` (Type-safe search params state manager).
  - _Rule:_ Prefer URL state for filters, pagination, and modal visibility (shareable links).

## 4. Backend, Database & API

- **ORM:** `Drizzle ORM` (Best-in-class TypeScript inference).
  - _Migration Tool:_ Drizzle Kit.
...
- **SDK:** `Vercel AI SDK` (Core/RSC/Streams).
  - _Rule:_ Use `streamText` and `streamUI` for performant UX.
