...
export async function saveQuestionsAction(questions: GeneratedQuestion[]) {
  ...
  const questionsToInsert: Database['public']['Tables']['questions']['Insert'][] = questions.map(
    (q) => {
      ...
      // Cast options result to unknown before Json
      const optionsJson = formatOptionsByType(q.type, q.options || undefined) as unknown as Database['public']['Tables']['questions']['Insert']['options'];

      return {
        user_id: user.id,
        content: dbContent as unknown as Database['public']['Tables']['questions']['Insert']['content'],
        options: optionsJson,
